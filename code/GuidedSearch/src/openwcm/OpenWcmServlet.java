...
public class OpenWcmServlet extends HttpServlet {
    ...
    private static final String PHOTO_GALLERY_ROOT_COLLECTION_ID = 
        "photoGalleryRootCollectionId";
    ...
    public static final String DOWNLOAD_URL_PREFIX = "portlet_provider_url";
    ...
    public String callWcmPlaceholder(String username, String dataFileContentId, 
                                     ...
                                     String siteId, ServletContext application, 
                                     Map<String, String> additionalArgs) throws ServletException {
        ...
        dataBinder.putLocal("placeholderDefinitionDocName", 
        ...
        dataBinder.putLocal("siteId", siteId);
    ...
    private IdcClient getIdcClient(ServletContext application) {
        UcmServletInfo info = 
            (UcmServletInfo)application.getAttribute(OpenWcmServlet.UCM_SERVLET_INFO);
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        String photoGalleryRootCollectionId = 
            config.getInitParameter(PHOTO_GALLERY_ROOT_COLLECTION_ID);
        ...
        String downloadUrlPrefix = config.getInitParameter(DOWNLOAD_URL_PREFIX);


        System.out.println("TEST " + testingUserName);
        String test = config.getInitParameter("testing");
        boolean testing = Boolean.parseBoolean(test);
        UcmServletInfo ucmServletInfo = 
            new UcmServletInfo(testingUserName, testing, servletName, 
                               servletContextName, 
                               photoGalleryRootCollectionId, idcClient, 
                               linkPrefix,downloadUrlPrefix);
        config.getServletContext().setAttribute(UCM_SERVLET_INFO, 
                                                ucmServletInfo);
    ...
    private String getUsername(ServletContext servletContext, 
    ...
    private UcmServletInfo getUcmServletInfo(ServletContext servletContext) {
        UcmServletInfo ucmServletInfo = 
            (UcmServletInfo)servletContext.getAttribute(OpenWcmServlet.UCM_SERVLET_INFO);
        return ucmServletInfo;
    }

    private void sendContentServerFileIntoResponse(String username, 
                                                   HttpServletResponse res, 
                                                   String contentId, 
                                                   IdcClient idcClient) throws ServletException, 
                                                                               IOException {
        ServiceResponse response = 
            getContentServerFile(username, contentId, idcClient);

        // Set the content type of the returned file
        res.setContentType(response.getHeader("Content-type"));
        res.setHeader("Content-Disposition", response.getHeader("Content-Disposition"));
    ...
    private ServiceResponse getContentServerFile(String username, 
                                                 String contentId, 
                                                 IdcClient idcClient) throws ServletException {
        IdcContext idcContext = new IdcContext(username);
        DataBinder dataBinder = idcClient.createBinder();
    ...
    private String getContentIdFromFilePath(String filePath) {
        ...
        String contentIdOnly = 
            lastElement.substring(0, lastElement.lastIndexOf("."));
        return contentIdOnly;
    }

    public static String formatUrls(ServletContext application, 
                                    String contentId) {
        UcmServletInfo ucmServletInfo = 
            (UcmServletInfo)application.getAttribute(UCM_SERVLET_INFO);
        String servletContextName = ucmServletInfo.getServletContextName();
        String servletName = ucmServletInfo.getServletName();

 //       String urlPrefix = ucmServletInfo.getDownloadUrlPrefix();
//        String url =    urlPrefix + "/" + servletContextName + "/" + servletName + "?" +   CONTENT_ID + "=" + contentId;

        String url  = ucmServletInfo.getDownloadUrlPrefix() + "?" + CONTENT_ID + "=" + contentId;
