"home":../../index.html > "training":../index.html > Maven
...
is a good plugin for maven into eclipse, make sure to set a jdk for the jvm ala: http://wiki.eclipse.org/Eclipse.ini#Specifying_the_JVM
...
* Goto your eclipse workspaces folder.
* Execute the following command replacing @com.mycompany.app@ with the domain name and app name, then change the artifact id to something like @app-web@.  
This will create a folder called @app-web@ in that directory

bc. mvn archetype:create -DgroupId=com.mycompany.app -DartifactId=app-web

h2(#eclipsify). Eclips'ify

@cd@ into the @app-web@ folder and make your project suitable for using with eclipse
