"home":../../index.html > "Training":../index.html > Cascading Style Sheets
...
h2. Linking style sheet to html
...
When specifying _classes_ in a *.css prefix them with a do (.) 
...
When specifying _div's_ in a css file, prefix it with a hash (#)
...
!images/box_model.gif!

@margin, border, padding@ plus @left, right, top, bottom@ as in @margin-left@.  @height, width@.  Here is a sample:
...
h2. Inline versus Block

Block elements follow each other vertically from top to bottom.  Examples: P, DIV or TABLE.

Inline elements flow from left to right first, then wrap around beneath.  Examples: B, I or SPAN and actual content like text and images.

!images/css-normal-flow.png! 

Any _inline_ or _block_ can be converted to the other by using the following:
...
Position attribute can take one of five values: absolute, relative, fixed, static, inherit
...
h3. Fixing the Collapsed Parent
...
h4. Solution 1: Float the container

The easiest way to fix this problem is to float the containing parent element:

!images/css7.png!

Now the container expands to fit all the child elements. But unfortunately this fix will only work in a limited number of circumstances, since floating the parent may have undesirable effects on your layout.  Now the container expands to fit all the child elements.
...
This article is useful for knowing how to apply _DRY(Dont Repeat Yourself)_ to css.
