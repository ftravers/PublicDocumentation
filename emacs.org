...
:hook - after one of the hooks is invoked
:bind - when bound key pressed
:command - when command used
:mode - a mode is invoked
:after - another package is loaded
:defer - catch-all load after init process
