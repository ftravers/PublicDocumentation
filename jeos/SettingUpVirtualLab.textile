...
* Use the OEL 5U3 repo. Next.  Add the @wget@ package. Next. Skip Additional Packages. Next. Click @Save and Build@.  It will take some time (maybe ~5-10 minutes) so relax and have a beer! :)
...
Export /OVS/downloads on server, import it to /mnt/vmsrv on client.  Please follow: "these instructions":applications.html#nfs to get it setup.
...
bc. system-config-securitylevel-tui --quiet --port=http --port=ssh --port=https
...
Since we've put quite a bit of work into this image already and don't want to repeat ourselves, lets take a backup of it.  From the VM server, shutdown the VM.

bc. xm shutdown ovmm_ovmtb

Ensure that the vm is not longer listed with the following command, you may need to wait a few seconds for the VM to shutdown.
...
Now since we chose to have a 12GB system drive this may take a while, in the future, I think I'll set this to be around 2GB and soft link the @/opt/ovmtb/tarballs@ folder directly to the NFS mount.  I'm impatient!  The result was a 183MB file...quite small and shareable!  Cool!  Thats an entire, modern OS in 183MB!  Fire the image back up, give it a minute to startup and ssh back into it.
...
The first one has an RPM we need next, and the second one has the OEL5U5 images we'll be using to stamp out new OEL5U5 images.

untar/unzip, then copy to the VM

bc. scp ovm-modify-jeos-1.1.0-2.el5.noarch.rpm root@alm.hk.oracle.com:/tmp

Go back onto the server and install the RPM.  But first we need to install some pre-requisits, then we can install ovmtb
...
That should do the trick.  Lets restart a couple of services for good measure and see if our web UI for TB is ready for us.
...
Recommend repointing these to the NFS drive so we can keep the OVMTB disk as small as possible. (2GB seems about the limit of small).  This makes backing up much quicker.
...
Now it can be a pain to download once to your desktop and then up to your server.  You can download directly from edelivery to your server with a cute little trick.  You can't just use @wget <url>@ because we need to login to edelivery and it gives our browser a cookie.  So do the following:
...
https://alm.hk.oracle.com:4443/OVS
