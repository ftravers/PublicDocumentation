...
These are _rapid_, _reference_ install instructions.  I refer to these directions each time I setup a new VM.  They are not meant as a tutorial, though as I begin to share these with other people that will probably be necessary and they may change to reflect that.

This document is used after you have create a new VM in OVMM.  After you've created that template there are some basics that all future VM's need and that is what this document is about.  We set the IP address, hostname, gateway, dns, etc..., plus a few other required things.

It's best that the machine be able to see the internet.

h1. Setup

http://olympus.stellent.com/xpedio/groups/public/documents/executable/oly_081696.tar = oel_conf_files.tar

"http://files.spicevan.com/vm/oel_conf_files2.tar":http://files.spicevan.com/vm/oel_conf_files2.tar

* copy file: oel_conf_files.tar to /

bc. 
[root@localhost ~]# cd /; tar xvf oel_conf_files.tar; rm -rf oel_conf_files.tar 
