...
As root user create new file "dbora" (init script for startup and shutdown the database) in /etc/init.d/ directory with following content:
...
The above seems to work for the DB and Listener (most important) but doesn't seem to do the trick for the EM Console...	
