...
You will often be setting up services that make use of certain ports
on your computer so it's important to understand how to configure the
firewall on that server to allow those ports to be open without
turning off the firewall completely.

* To add a new port, run the following and

bc. 
[root@ucm3 ~]# /usr/bin/system-config-securitylevel-tui --quiet
--port=http --port=ssh --port=https --port=138:udp --port=445:tcp

Choose Customize, add a line like: 8888:tcp, to the other ports
section, OK, OK.  Restart the firewall with:

bc. [root@ucm3 ~]# service iptables restart

The following command will show you if your changes worked...

bc. [root@ucm3 ~]# cat /etc/sysconfig/iptables

The rest below is just additional background info about the firewall:

First backup your current IP Tables settings:

bc. [root@hp4 etc]# cp /etc/sysconfig/iptables /etc/sysconfig/iptables.orig 
	
Restore original tables:

bc. [root@hp2 ~]# cp /etc/sysconfig/iptables.orig /etc/sysconfig/iptables
	
Start, stop, restart the iptables ( will reload /etc/sysconfig/iptables rules )

bc. [root@hp2 ~]# service iptables [restart|start|stop]
...
bc. [root@hp2 ~]# /sbin/service iptables save
...
h2. General form of command is:

bc. iptables -A CHAIN -p tcp/udp [options] -j ACTION
